...
function spaNavigate(path) {
    ...
    const target = routes[path] || (path.startsWith('/') ? path : '/' + path.replace('.html', ''));
...
function isSpaPath(segment) {
    return window.location.pathname.includes(segment) || window.location.pathname === segment.replace('.html', '') || window.location.pathname === '/' + segment.replace('.html', '');
}
        ...
        if (data.success) {
            ...
            setTimeout(() => {
                ...
                if (data.user.role === 'admin') {
                    spaNavigate('admin.html');
                } else {
                    if (isSpaPath('login')) {
                        spaNavigate('index.html');
                    } else if (typeof window.__reactNavigate === 'function') {
    ...
    if (!userBtn.parentElement.classList.contains('profile-dropdown-container')) {
        ...
        const dropdownHtml = `
            <div id="profileDropdownMenu" class="profile-dropdown-menu">
                ...
                <a href="/profile" class="dropdown-item"><i class="fa-regular fa-user"></i> My Profile</a>
                <a href="/track-order" class="dropdown-item"><i class="fa-solid fa-location-dot"></i> Track Order</a>
                <hr class="dropdown-divider">
...
(function() {
    ...
    const isLoggedIn = localStorage.getItem('isLoggedIn') === 'true';
    if (pathname.includes('profile') && !pathname.includes('login')) {
        if (!isLoggedIn) {
            spaNavigate('login.html');
        }
    }
