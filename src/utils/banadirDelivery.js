...
export const WAREHOUSE_DISTRICT = 'Hodan';
...
export function deliveryFeeFromKm(km) {
...
export function buildDefaultDeliveryFees() {
...
export function getDistrictKm(districtName) {
...
export function feeForDistrictName(districtName) {
  const km = getDistrictKm(districtName);
  if (km == null) return null;
  return deliveryFeeFromKm(km);
}

export function mergeBanadirDeliveryFees(existing = []) {
...
/** For driver apply / selects */
export const MOGADISHU_DISTRICT_OPTIONS = BANADIR_DISTRICTS.map(({ name }) => ({
  value: name,
  label: name,
}));
